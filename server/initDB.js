...
db.serialize(function () {
  ...
  db.run(  
    ...
    db.run(
      "INSERT OR IGNORE INTO HIKE(id,title,length_kms,expected_mins,ascendent_meters,difficulty)\
       VALUES (1, 'Path to ROCCIAMELONE', 9, 420, 3538, 'Professional Hiker'),\
              (2, 'Ascesa al Rifugio Savona', 10.3, 244, 610, 'Hiker'),\
              (3, 'Salita al Monte Antoroto', 17, 444, 1090, 'Professional Hiker'),\
              (4, 'Salita al Bric Mindino e al Colle di Prato Rotondo', 9.2, 218, 610, 'Hiker'),\
              (5, 'Colletta di Castelbianco Loop from Veravo', 5.7, 105, 206, 'Tourist'),\
              (6, 'Chiesa di Santa Libera Loop from Losano', 4.6, 80, 122, 'Tourist')"
    );

    db.run(
      "INSERT OR IGNORE INTO POINT(id, latitude, longitude )\
  ...
  db.run(
    "INSERT OR IGNORE INTO PARKING_LOT(id,name,point_id)\
   ...
   db.run(
    "INSERT OR IGNORE INTO HUT(id,name,point_id)\
