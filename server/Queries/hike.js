...
exports.deleteHutForHike = (hike_id, hut_id) => {
        const sql = "DELETE * FROM HIKE_HUT WHERE hike_id = ?, hut_id = ?"
        return db.run(sql, [hike_id, hut_id]);
}

exports.deleteParkForHike = (hike_id, park_id) => {
        const sql = "DELETE * FROM HIKE_PARKING WHERE hike_id = ?, parking_id = ?"
        return db.run(sql, [hike_id, park_id]);
}

exports.insertHutForHike = async (hike_id, hut_id) => {
        const sql = "INSERT INTO HIKE_HUT(hike_id , hut_id) VALUES(?, ?)"
        return db.insert(sql, [hike_id, hut_id]);
}

exports.insertParkForHike = async (hike_id, park_id) => {
        const sql = "INSERT INTO HIKE_PARKING(hike_id , parking_id) VALUES(?, ?)"
        return db.insert(sql, [hike_id, park_id]);
}
